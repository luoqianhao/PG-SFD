...
class ClassificationModule(nn.Module):
    ...
    def forward(
        ...
        if anomaly_prototypes is not None:
            ...
            p_normal_AD = anomaly_prototypes.detach().mean(1)  # [B, D], learned without supervision.
        ...
        outputs = (cls_logits, cls_similarities, class_proto)
        if return_features:
            # f_cls used by the assumption-explicit Eq. (7) implementation.
            # The default return contract remains unchanged for other callers.
            return (*outputs, global_feature)
        return outputs
