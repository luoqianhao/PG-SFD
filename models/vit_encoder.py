...
def load(name):
    ...
    if "mae" in name:
        if 'base' in name:
            model = mae_vit_base_patch16_dec512d8b(img_size=224)
        ckpt_pth = "./mae/weight/mae_pretrain_vit_base.pth"
    ...
    if "beitv2" in name:
        ...
        ckpt_pth = "./beit/weight/beitv2_base_patch16_224_pt1k_ft21k.pth"
        # ckpt_pth = download_cached_file(
        #     f"https://github.com/addf400/files/releases/download/BEiT-v2/beitv2_base_patch16_224_pt1k_ft21k.pth")
        state_dict = torch.load(ckpt_pth, map_location='cpu')['model']
        ...
        model = beitv2_base_patch16_224(pretrained=False)
        # ckpt_pth = download_cached_file(
        #     f"https://github.com/addf400/files/releases/download/v1.0/beit_base_patch16_224_pt22k_ft22k.pth")
        ckpt_pth = "./beit/weight/beit_base_patch16_224_pt22k_ft22k.pth"
        state_dict = torch.load(ckpt_pth, map_location='cpu')['model']
    ...
    if "deit" in name:
        arch, patchsize = name.split("_")[-2], name.split("_")[-1]
        model = vision_transformer.__dict__[f'vit_{arch}'](patch_size=int(patchsize))
        if 'v3' in name:   #deitv3
            model = deit_small_patch16_LS(pretrained=False)
            if arch == 'small':
                ckpt_pth = "./backbones/weights/deit_3_small_224_21k.pth"
        else:   #deit
            if arch == "base":
                ckpt_pth = download_cached_file(
                ...
                ckpt_pth = "./backbones/weights/deit_small_patch16_224-cd65a155.pth"
            else:
                raise ValueError("Invalid type of architecture. It must be either 'small' or 'base'.")
...
def load_dinov3(name):
    ...
    if 'vits16' in name:
        arch = 'small'
        model = vision_transformer_dinov3.__dict__[f'vit_{arch}'](patch_size=int(patchsize), img_size=518,
                                                                    mask_k_bias=True, layerscale_init=1e-5,
                                                                    n_storage_tokens=4)
        ckpt_pth = './dinov3/weight/dinov3_vits16_pretrain_lvd1689m-08c60483.pth'
