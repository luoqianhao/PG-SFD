...
# from mae.vit import PatchEmbed, Block
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        pos = self._build_abs_pos(self.cls_token.size(-1), x.shape[1], x.device, x.dtype, cls_token=True)
        x = x + pos[:, 1:, :]
        # masking: length -> length * mask_ratio
        x, mask, ids_restore = self.random_masking(x, mask_ratio)

        # append cls token
        cls_tokens = self.cls_token.expand(x.shape[0], -1, -1) + pos[:, 1:, :]
